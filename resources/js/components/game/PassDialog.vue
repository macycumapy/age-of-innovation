...
<template>
    <Dialog v-model:open="isOpen">
        <DialogContent class="sm:max-w-lg">
            <Form v-bind="PassController.form(gameId)" class="contents" #default="{ errors, processing }" @success="isOpen = false">
                <input type="hidden" name="round_bonus" :value="selectedRoundBonus ?? ''" />
                <DialogHeader>
                    <DialogTitle>Спасовать?</DialogTitle>
                    <DialogDescription>
                        Выберите новый жетон бонуса раунда. Текущий жетон вернётся в общий пул.
                    </DialogDescription>
                </DialogHeader>

                <div v-if="availableActions.length > 0" class="rounded-lg border border-amber-500/50 bg-amber-500/10 px-3 py-2 text-sm">
                        ...
                        <Tooltip v-for="offer in offers" :key="offer.roundBonus">
                            <TooltipTrigger as-child>
                                <button
                                    ...
                                    :class="selectedRoundBonus === offer.roundBonus ? 'border-primary ring-2 ring-primary' : 'border-muted'"
                                    :aria-pressed="selectedRoundBonus === offer.roundBonus"
                                    @click="selectedRoundBonus = offer.roundBonus"
                                >
                                    <img :src="roundBonusImage(offer.roundBonus)" :alt="`Бонус раунда ${offer.roundBonus}`" class="h-auto w-full rounded-md" />
                                    <span v-if="offer.coins > 0" class="absolute top-3 right-3 grid size-9 place-items-center">
